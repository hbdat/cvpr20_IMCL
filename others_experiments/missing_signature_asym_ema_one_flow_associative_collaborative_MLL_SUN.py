# -*- coding: utf-8 -*-
"""
Created on Tue Sep  4 19:21:45 2018
...
import pdb
from global_setting_SUN import fractions,list_alphas_colaborative,list_alphas_feature
#%% logging level
#global_setting_SUN.n_cycles//=3
global_setting_SUN.learning_rate_base=0.01
global_setting_SUN.batch_size=32
#%%
def experiment_cond_success_frac():
    return True#fraction!=0.2
def experiment_cond_success():
    index = idx_alpha_colaborative*len(list_alphas_colaborative)+idx_alpha_feature*len(list_alphas_feature)+idx_alpha_regularizer*len(global_setting_SUN.regularizers)
...
for idx_alpha_colaborative,alpha_colaborative_o in enumerate(list_alphas_colaborative):
    for idx_alpha_feature,alpha_feature_o in enumerate(list_alphas_feature):
        for idx_alpha_regularizer,alpha_regularizer_o in enumerate(global_setting_SUN.regularizers):
...
is_balance = True
if is_balance:
    global_setting_SUN.saturated_Thetas_model='./result/SUN_missing_signature_balance_1_inv_time_baseline_0.001_32_0.8_signal_str_0.5_1000_1_GPU_1_thresCoeff_1e-06_stamp_1536086662.1684954/model_signatures.npz'
else:
    global_setting_SUN.saturated_Thetas_model='./result/SUN_missing_signature_balance_0_inv_time_baseline_0.001_32_0.8_signal_str_0.5_1000_1_GPU_1_thresCoeff_1e-06_stamp_1536087119.8119936/model_signatures.npz'

os.environ["CUDA_VISIBLE_DEVICES"]="{}".format(idx_GPU)
name_template='v2_SUN_missing_signature_balance_'+str(0+is_balance)+'_{}_{}_{}_signal_str_{}_{}_frac_{}_GPU_{}_thresCoeff_{}_c_{}_stamp_{}'
...
val_attributes = val_attributes[:,:,-1]
...
with tf.variable_scope("sparse_coding_OMP"):
    A,P,R_F= colaborative_loss.OMP_asym_sigmoid_Feature_Graph(Theta,F,sparse_dict,atttibute_f,sparse_dict_Attributes_f,
                                                         global_setting_SUN.k,part_G_var,alpha_colaborative_var,
                                                         alpha_feature_var,parallel_iterations,
                                                         c,global_setting_SUN.thresold_coeff)
...
#    R= tf.Print(R,[tf.norm(F)],'Place 2:')
    loss_colaborative=tf.square(tf.norm(R))*1.0/global_setting_SUN.batch_size
...
#%%
def append_info(m_AP,m_error,m_error_q,loss_value,loss_logistic_value,lr_v):
    res_mAP[index] = m_AP
...
for fraction_idx,fraction  in zip(ranks,fractions):
    name = name_template.format(global_setting_SUN.learning_rate_base,global_setting_SUN.batch_size,global_setting_SUN.decay_rate_cond
                                               ,global_setting_SUN.signal_strength,global_setting_SUN.n_cycles,fraction,
                                               idx_GPU,global_setting_SUN.thresold_coeff,c,time.time())
    if not experiment_cond_success_frac():
        print('skip')
        continue
    data=np.load(global_setting_SUN.saturated_Thetas_model)
...
#                        print('{} alpha: colaborative {} feature {} regularizer {}'.format(name,alpha_colaborative,alpha_feature,alpha_regularizer))
                        ...
                        signature_comp_no_mean=signature_completion(Label_completion_v,sparse_dict_label_v,signature_q,quantization)
                        signature_comp = signature_comp_no_mean#-np.mean(signature_comp_no_mean,axis=0)[np.newaxis,:]
...
#                            expon_moving_avg_new = expon_moving_avg_new*(1-global_setting_SUN.signal_strength)+m_AP*global_setting_SUN.signal_strength
#                            if expon_moving_avg_new<expon_moving_avg_old and learning_rate.eval() >= global_setting_SUN.limit_learning_rate and m <= 0:
#                                print('Adjust learning rate')
#                                sess.run(op_assign_learning_rate,{learning_rate_fh:learning_rate.eval()*global_setting_SUN.decay_rate_cond})
#                                m = 2
#                            m -= 1
                        append_info(m_AP,m_error,m_error_q,loss_value,logistic_v,lr_v)
...
##                            pdb.set_trace()
#                            if global_setting_SUN.early_stopping and m_AP >= np.max(res_mAP):
#                                print('-'*30)
#                                print('Save best')
